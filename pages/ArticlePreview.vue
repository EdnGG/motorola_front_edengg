<template>
	<div id="article" class="container-fluid home">
		<section
			...
			:style="{
				'background-image': `url(https://api.motorolanews.com/cache/image?url=${article.featured_image})`,
				height: `${height_img_main}px`,
				'min-height': `100px`,
			}"
		>
			<div
				id="s_target_main"
				class="container-fluid position-absolute bg-20-primary w-100"
				:style="{ height: `598px` }"
			>
				>
				...
				<div class="container-fluid px-0">
					<div class="row justify-content-center">
						...
						<div class="col-12 col-md-7 py-5 pr-0 pr-md-5">
								...
								<div class="col-12 px-0">
									...
									>
										...
										<img
											...
											class="img-fluid"
											@click="shareMedia(item_social.id)"
							...
							<div class="pb-4">
								<div class="d-inline-block mr-3">
									<span
										class="mo--description montserrat-bold"
									>
										{{ $t("share") }}
									</span>
								</div>
						...
						<div class="col-1 col-md-3 d-none d-md-block pt-5 pb-0">
							<div class="row">
								...
								<div
									...
									:key="`tn_${index_news}`"
									@click.once="
										redirect('article', news.id, news.title)
									"
									style="cursor: pointer"
								>
					...
					<div class="row justify-content-center pb-5">
						<div class="col-12 col-md-10 my-4">
							<span
								class="mo--title-sm montserrat-bold text-danger"
							>
								{{ $t("related_news") }}
							</span>
						</div>
						...
						<div class="col-12 col-md-10 p-0" >
							<div class="row justify-content-center" v-if="post !== null">
								<div
									...
									v-for="(
										item_rn, index_rn
									) in post.related_news"
									:key="index_rn"
									class="
										col-10 col-sm-6 col-md-4 col-lg-4
										my-2
										d-flex
									"
								>
...
export default {
		...
		return $seo({
			...
			openGraph: {
				...
				description: post.news.body
					...
					.slice(0, 90),
				url: process.browser
					? `https://motorolanews.com${location.pathname}`
					: null,
				type: "website",
				multi: true,
				image: {
	...
	methods: {
		...
		shareFacebook(params) {
            ...
            params.url = `${params.url}&shared=whatsapp`
		...
		async initConfig(other_id = null) {
			...
			let data = this.$jwt.verify(
				this.$envvar.seed,
				//this.$route.params.hash.toString()
				this.$storage.getUniversal('hash_id')
			);

			data = this.$aes.decrypt(Buffer.from(data, "base64"));
		...
		async redirect(route_name, id, title) {
			this.$store.commit("setLoad", {
				show: true,
				title: "Loading...",
			});
			...
			let hash_id = this.$aes.encrypt(JSON.stringify(id_local));
			hash_id = this.$jwt.hash(this.$envvar.seed, hash_id);

			this.$storage.setUniversal('hash_id', hash_id)
...
;
		...
		async downloadResource() {
			...
			// Decrypt JWT
			let hash_id = this.$jwt.verify(
				this.$envvar.seed,
				this.$route.params.hash.toString()
			);

			let data = this.$aes.decrypt(
				Buffer.from(hash_id.toString(), "base64")
			);
			data = JSON.parse(data);
			data = this.$aes.encrypt(JSON.stringify(data));
	...
	async asyncData(app) {
		...
		let path =  app.route.fullPath;
		let title = path.replace('/article-preview/', '')
        ...
        console.log(`pressbox/news/article?data=${base64.encode(data)}`)
